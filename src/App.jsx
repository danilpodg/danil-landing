...
function IconBadge({ children, small = false }) {
  return (
    <span
      className={`inline-flex shrink-0 items-center justify-center rounded-2xl border border-violet-100 bg-violet-50 text-violet-600 shadow-sm transition duration-300 group-hover:-translate-y-1 group-hover:bg-violet-100 ${
...
function LandingPreview({ project }) {
  return (
    <div className="mb-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/70 transition duration-500 group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-violet-200/60">
    ...
    <>
      ...
      <main className="min-h-dvh overflow-x-hidden bg-white text-slate-950">
        <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(139,92,246,0.16),transparent_32%),linear-gradient(180deg,#ffffff_0%,#fafafa_100%)]">
          <div className="absolute right-[-140px] top-[-140px] h-96 w-96 rounded-full bg-violet-200/50 blur-3xl" />
          <div className="absolute bottom-[-120px] left-[-120px] h-80 w-80 rounded-full bg-fuchsia-100 blur-3xl" />
          ...
          <header className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
            ...
            <nav className="hidden items-center gap-8 text-sm font-medium text-slate-500 md:flex">
              <a href="#services" className="transition hover:text-violet-600">Услуги</a>
              <a href="#process" className="transition hover:text-violet-600">Этапы</a>
              <a href="#projects" className="transition hover:text-violet-600">Форматы</a>
              <a href="#contacts" className="transition hover:text-violet-600">Контакты</a>
            </nav>

            <a href="#contacts" className="rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-xl shadow-slate-300/70 transition duration-300 hover:-translate-y-0.5 hover:bg-violet-700 hover:shadow-violet-200">
              Обсудить проект
            </a>
          </header>
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-14 px-6 pb-24 pt-14 md:grid-cols-2 md:pb-28 md:pt-24">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-violet-100 bg-white px-4 py-2 text-sm font-medium text-violet-700 shadow-sm">
              ...
              <div className="mt-9 flex flex-col gap-4 sm:flex-row">
                <a href="#contacts" className="inline-flex items-center justify-center gap-2 rounded-full bg-violet-600 px-7 py-4 font-semibold text-white shadow-xl shadow-violet-200 transition duration-300 hover:-translate-y-1 hover:bg-violet-700 hover:shadow-violet-300">
                  Оставить заявку <span aria-hidden="true">→</span>
                </a>
                <a href="#projects" className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-7 py-4 font-semibold text-slate-950 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-violet-200 hover:text-violet-700 hover:shadow-xl hover:shadow-slate-200/70">
                  Смотреть форматы
                </a>
              </div>
            </div>
            ...
            <div className="relative">
              <div className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-2xl shadow-slate-200/80">
                <div className="flex h-[340px] items-center justify-center rounded-[1.5rem] bg-gradient-to-br from-violet-50 via-white to-fuchsia-50 p-8 text-center ring-1 ring-slate-100 md:h-[480px]">
                  <div>
                    <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-[2rem] bg-slate-950 text-5xl shadow-2xl shadow-violet-200" aria-hidden="true">
                      🖥️
                    </div>
              ...
              <div className="absolute -bottom-8 -left-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl shadow-slate-200/80">
                <p className="text-4xl font-black text-violet-600">1</p>
        ...
        <section id="services" className="mx-auto max-w-7xl px-6 py-24">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-violet-600">Услуги</p>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {services.map((service) => (
              <article key={service.title} className="group rounded-3xl border border-slate-200 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-2 hover:border-violet-200 hover:shadow-2xl hover:shadow-violet-100">
                <div className="mb-6">
        ...
        <section id="process" className="bg-slate-50 py-24">
          <div className="mx-auto max-w-7xl px-6">
            <div className="grid gap-12 md:grid-cols-2">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-violet-600">Процесс</p>
                <h2 className="mt-4 text-4xl font-black text-slate-950 md:text-5xl">Как проходит работа</h2>
              </div>
              <div className="space-y-5">
                {processSteps.map((step, index) => (
                  <div key={step} className="flex gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-violet-200 hover:shadow-xl hover:shadow-slate-200/70">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-violet-600 font-bold text-white">{index + 1}</div>
        ...
        <section id="projects" className="mx-auto max-w-7xl px-6 py-24">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-violet-600">Форматы</p>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {projects.map((project) => (
              <div key={project.title} className="group rounded-[2rem] border border-slate-200 bg-white p-7 shadow-sm transition duration-300 hover:-translate-y-2 hover:border-violet-200 hover:shadow-2xl hover:shadow-violet-100">
                <LandingPreview project={project} />
                <p className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-violet-500">{project.category}</p>
                <h3 className="text-2xl font-black text-slate-950 transition duration-300 group-hover:text-violet-700">{project.title}</h3>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-24">
          <div className="rounded-[2rem] border border-violet-100 bg-gradient-to-br from-violet-50 to-white p-8 shadow-xl shadow-violet-100/70 md:p-12">
          ...
          <div className="mx-auto grid max-w-7xl gap-12 px-6 py-24 md:grid-cols-2">
            <div>
              ...
              <div className="mt-8 space-y-4 text-slate-700">
                ...
                <p className="flex items-center gap-3"><span aria-hidden="true">✉️</span> <a href="mailto:danilpodgayniy@gmail.com" className="transition hover:text-violet-600">danilpodgayniy@gmail.com</a></p>
            ...
            <form action="https://formspree.io/f/mpqkgqgn" method="POST" className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-200/80">
              ...
              <input id="name" name="name" required className="mb-4 w-full rounded-2xl border border-slate-200 bg-white px-5 py-4 outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-100" placeholder="Например, Алексей" />

              <label className="mb-2 block text-sm font-semibold text-slate-700" htmlFor="phone">Телефон или Telegram</label>
              <input id="phone" name="phone" required className="mb-4 w-full rounded-2xl border border-slate-200 bg-white px-5 py-4 outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-100" placeholder="+7 или @username" />

              <label className="mb-2 block text-sm font-semibold text-slate-700" htmlFor="message">Что нужно сделать?</label>
              <textarea id="message" name="message" required className="mb-4 h-32 w-full resize-none rounded-2xl border border-slate-200 bg-white px-5 py-4 outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-100" placeholder="Например: нужен лендинг для услуги ремонта, доставки, дизайна, портфолио..." />

              <button type="submit" className="w-full rounded-full bg-violet-600 px-7 py-4 font-bold text-white shadow-xl shadow-violet-200 transition duration-300 hover:-translate-y-1 hover:bg-violet-700 hover:shadow-violet-300">
                Отправить заявку
              </button>
